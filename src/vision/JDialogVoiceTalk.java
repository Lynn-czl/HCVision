...
public class JDialogVoiceTalk extends javax.swing.JDialog {
	...
	private HCNetSDK.NET_DVR_WORKSTATE_V30 m_strWorkState;// 工作状态
	private FVoiceDataCallBack fVoiceDataCallBack;// 回调函数
	private int m_iSel;// 音频通道
	private boolean m_bInitialed;// 对话框是否已初始化
	...
	{// GEN-HEADEREND:event_jButtonStartActionPerformed
		...
		Vision.g_lVoiceHandle = hCNetSDK.NET_DVR_StartVoiceCom_V30(m_lUserID, m_iSel + 1,
				jComboBoxDataType.getSelectedIndex() == 1, fVoiceDataCallBack, null);
		if (Vision.g_lVoiceHandle.intValue() == -1) {
	...
	{// GEN-HEADEREND:event_jButtonStopActionPerformed
		if (Vision.g_lVoiceHandle.intValue() >= 0) {
			if (hCNetSDK.NET_DVR_StopVoiceCom(Vision.g_lVoiceHandle)) {
				Vision.g_lVoiceHandle.setValue(-1);
			} else {
