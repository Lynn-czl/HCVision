...
class JDialogPlayBack extends javax.swing.JDialog {
	...
	private NativeLong m_lPlayBackHandle;// 回放句柄
	...
	private IntByReference m_nFileTime;// 文件总时间
	private IntByReference m_nTotalFrames;// 文件总帧数

	private int m_nTotalSecond;// 总秒数
	private int m_nTotalMinute;// 总分钟数
	private int m_nTotalHour;// 总小时

	private boolean m_bGetMaxTime;// 是否已获得总播放时间,在计时器响函数里,只需要调用一次
	private boolean m_bSaveFile;// 是否在保存文件
	...
	/**
	 * 函数: "Pause" 按钮单击相应函数 函数描述: 暂停播放 函数: "退出" 按钮单击相应函数 函数描述: 销毁对话框
	 */
	...
	/**
	 * DownloadTask 类描述: 下载定时器响应函数
	 * 
	 * @author WSL
	 *
	 */
	class DownloadTask extends java.util.TimerTask {// 定时器函数
		@Override
		public void run() {
	...
	/**
	 * PlaybackTask 类描述: 回放定时器响应函数
	 * 
	 * @author WSL
	 *
	 */
	class PlaybackTask extends java.util.TimerTask {// 定时器函数
		@Override
		public void run() {
	...
	private void initComponents() {
		...
		jComboBoxRecodType.setModel(new javax.swing.DefaultComboBoxModel(
				new String[] { "全部", "定时录像", "移动侦测", "报警触发", "报警|动测", "报警&动测", "命令触发", "手动录像" }));
